...
export default function AdminStaff() {
  ...
  const save = async (event) => {
    ...
    try {
      if (form.id) {
        const { error } = await supabase.functions.invoke('manage-staff', { body: { action: 'update', id: form.id, full_name: form.full_name, email: form.email, role: form.role, permissions: form.permissions } })
        if (error) throw error
      } else {
        if (!form.password) throw new Error('A temporary password is required.')
        const { error } = await supabase.functions.invoke('create-staff', { body: { ...form, permissions: form.permissions } })
        if (error) throw error
      }
      await load()
      setMode(null)
  ...
  const remove = async (member) => {
    ...
    const { error } = await supabase.functions.invoke('manage-staff', { body: { action: 'delete', id: member.id } })
    if (error) setMessage(error.message)
