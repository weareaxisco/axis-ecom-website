...
export default function AdminStaff() {
  ...
  const openEdit = (member) => { setForm({ ...member, password: '', custom_alias: member.custom_alias || '', permissions: permissionFlags(member) }); setMode('edit') }
  ...
  const save = async (event) => {
    ...
    try {
      if (form.id) {
        const updateBody = { action: 'update', id: form.id, full_name: form.full_name, email: form.email, custom_alias: form.custom_alias, role: form.role, permissions: form.permissions, permissionFlags: form.permissions }
        const { data, error } = await functionOptions(updateBody)
        if (error) {
          console.error('Manage Staff Error:', error)
          const { error: fallbackError } = await supabase.from('profiles').update({ full_name: form.full_name, email: form.email, custom_alias: form.custom_alias, role: form.role, permissions: form.permissions, ...form.permissions }).eq('id', form.id)
        ...
        const { data, error } = await functionOptions({ action: 'create', ...form, permissions: form.permissions, permissionFlags: form.permissions })
        if (error) {
          console.error('Manage Staff Error:', error)
