...
export default function TaxonomyCombobox({ label, value = '', options = [], onChange, onSelect, selectedValues = [], onRemove, hideLabel = false }) {
  ...
  const input = multiple ? <div className="flex min-h-[44px] w-full flex-wrap items-center gap-1.5 rounded-none border border-neutral-800 bg-neutral-900 p-2 focus-within:border-amber-400">
      ...
      </div> : <input value={value} onFocus={() => setOpen(true)} onChange={(event) => { onChange(event.target.value); setOpen(true) }} className="block h-11 w-full border border-neutral-800 bg-neutral-900 px-3 text-sm normal-case tracking-normal outline-none focus:border-amber-400" aria-expanded={open} aria-autocomplete="list" />
  return <div ref={ref} className="relative block w-full min-w-0">
    {hideLabel ? input : <label className="block w-full text-xs uppercase tracking-wider text-neutral-400">{label}{input}</label>}
