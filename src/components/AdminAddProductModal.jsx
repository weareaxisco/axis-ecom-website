...
export default function AdminAddProductModal({ onClose, onCreated }) {
  ...
  return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 p-4"><form onSubmit={submit} className="max-h-[90vh] w-full max-w-2xl overflow-y-auto border border-neutral-800 bg-neutral-950 p-6 text-white"><div className="flex justify-between"><h2 className="font-serif text-2xl uppercase tracking-widest">{t('addNewCreationTitle')}</h2><button type="button" onClick={onClose} aria-label={t('close')}><X /></button></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-[10px] uppercase tracking-widest text-neutral-400 sm:col-span-2">{t('titleName')}<input required value={form.name} onChange={update('name')} className={input} /></label><label className="text-[10px] uppercase tracking-widest text-neutral-400">{t('category')}<select value={form.category} onChange={update('category')} className={input}>{categories.map((item) => <option key={item}>{item}</option>)}</select></label><label className="text-[10px] uppercase tracking-widest text-neutral-400">{t('collection')}<select value={form.collection} onChange={update('collection')} className={input}>{collections.map((item) => <option key={item}>{item}</option>)}</select></label>{attributeSelect('metal')}{attributeSelect('gender')}{attributeSelect('shape')}{attributeSelect('novelty')}<label className="text-[10px] uppercase tracking-widest text-neutral-400">{t('priceInDh')}<input required type="number" min="0" value={form.price_dh} onChange={update('price_dh')} className={input} /></label><label className="text-[10px] uppercase tracking-widest text-neutral-400">{t('stockQuantity')}<input type="number" min="0" value={form.stock} onChange={update('stock')} className={input} /></label><label className="text-[10px] uppercase tracking-widest text-neutral-400 sm:col-span-2">{t('imageUrls')} <span className="normal-case">({t('onePerLine')})</span><textarea value={form.images} onChange={update('images')} className={`${input} h-20`} />{form.images.split(/\r?\n|,/).filter(Boolean).map((src) => <img key={src} src={src.trim()} alt="" className="mt-2 mr-2 inline-block h-14 w-14 object-cover" />)}</label><label className="text-[10px] uppercase tracking-widest text-neutral-400 sm:col-span-2">{t('description')}<textarea value={form.description} onChange={update('description')} className={`${input} h-20`} /></label><label className="text-[10px] uppercase tracking-widest text-neutral-400 sm:col-span-2">{t('materialSpecifications')}<input value={form.material} onChange={update('material')} className={input} /></label></div><label className="mt-5 flex items-center gap-3 text-xs text-amber-300"><input type="checkbox" checked={form.onsite_only} onChange={update('onsite_only')} className="accent-amber-500" />{t('boutiqueExclusive')}</label>{error && <p className="mt-4 text-xs text-rose-400">{error}</p>}<button type="submit" className="mt-6 w-full bg-amber-500 py-3 text-xs font-semibold uppercase tracking-widest text-black">{t('createCreation')}</button></form></div>
}
