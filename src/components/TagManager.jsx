...
export default function TagManager({ value = [], onChange }) {
  ...
  return <div className="text-[10px] uppercase tracking-widest text-neutral-400">Tags
    ...
    <div className="mt-2 flex gap-2"><input value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); add() } }} placeholder="Add a tag" className="h-11 min-w-0 flex-1 border border-neutral-800 bg-neutral-900 px-3 text-sm normal-case tracking-normal outline-none focus:border-amber-400" /><button type="button" onClick={() => add()} aria-label="Add tag" className="h-11 border border-neutral-700 px-3 text-amber-300"><Plus size={15} /></button></div>
