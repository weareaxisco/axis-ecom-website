...
const defaultPopularSearches = ['Happy Sport', 'Ice Cube', 'Diamond', 'Timepieces']
...
export default function DesktopSearchDropdown({
  ...
  return (
    <div className="fixed inset-0 z-40 hidden md:block" role="dialog" aria-modal="true" aria-label="Desktop search">
      ...
      <section className="fixed left-0 right-0 top-[152px] z-40 max-h-[55vh] w-full overflow-y-auto border-b border-neutral-800 bg-neutral-950/98 text-white shadow-2xl backdrop-blur-2xl transition-all duration-300">
        <div className="mx-auto max-w-7xl px-6">
          <div className="flex w-full justify-center border-b border-neutral-800/80 pb-6 pt-8">
            <div className="relative w-full max-w-2xl px-4">
              <div className="relative flex items-center border border-neutral-800 bg-neutral-900/90 px-4 py-3 transition-all focus-within:border-amber-500/70">
                <Search className="mr-3 h-5 w-5 flex-shrink-0 text-neutral-400" />
                <input
                  ref={inputRef}
                  type="text"
                  autoFocus
                  ...
                  placeholder="Search creations, timepieces, fine jewelry..."
                  className="w-full bg-transparent font-serif text-base text-white outline-none placeholder:text-neutral-500 md:text-lg"
                />
                {isDebouncing && <span className="mr-2 h-2 w-2 animate-pulse rounded-full bg-amber-400" aria-label="Searching" />}
                {searchQuery && (
                  <button type="button" aria-label="Clear search" onClick={() => updateSearchQuery('')} className="p-1 text-neutral-400 hover:text-white">
                    <X className="h-4 w-4" />
                  </button>
                )}
              </div>
          ...
          <div className="grid grid-cols-12 gap-8 py-6">
            <div className="col-span-12 space-y-4 md:col-span-4">
              <div>
                <h3 className="mb-3 text-[11px] font-mono tracking-[0.2em] text-amber-400/90 uppercase">Popular</h3>
                <div className="flex flex-wrap gap-x-4 gap-y-2">
                  {popularSearches.map((item) => (
                    <button key={item} type="button" onClick={() => selectSearch(item)} className="text-xs uppercase tracking-widest text-neutral-400 transition-colors hover:text-amber-300">{item}</button>
                  ))}
                </div>
              </div>
              <div>
                <h3 className="mb-3 text-[11px] font-mono tracking-[0.2em] text-amber-400/90 uppercase">Collections</h3>
            ...
            <div className="col-span-12 md:col-span-8">
              <h3 className="mb-3 flex items-center justify-between text-[11px] font-mono tracking-[0.2em] text-amber-400/90 uppercase">
