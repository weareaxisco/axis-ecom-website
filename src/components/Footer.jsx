...
function SectionLinks({ title, links, mobile = false }) {
  ...
  return <div className={mobile ? 'border-b border-[var(--border-subtle)]' : ''}><button type="button" aria-expanded={mobile ? isOpen : undefined} onClick={() => setIsOpen((open) => !open)} className={`${mobile ? 'flex min-h-12 w-full items-center justify-between py-4 focus-visible:outline focus-visible:outline-2 focus-visible:outline-amber-400' : 'mb-5 block cursor-default text-left'} text-[10px] uppercase tracking-[0.2em]`}>{title}{mobile && <ChevronDown size={15} strokeWidth={1.25} className={`transition-transform ${isOpen ? 'rotate-180' : ''}`} />}</button><div className={`${mobile ? `grid transition-[grid-template-rows] duration-300 ${isOpen ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}` : ''}`}><ul className={`${mobile ? 'overflow-hidden' : ''} space-y-3 text-xs opacity-70`}>{links.map(([label, href], index) => <li key={`${href}-${index}`}><Link to={href} className="inline-flex min-h-10 items-center py-2 hover:text-amber-300 focus-visible:outline focus-visible:outline-2 focus-visible:outline-amber-400">{label}</Link></li>)}</ul></div></div>
}
...
export default function Footer() {
  ...
  return <><footer className="border-t border-[var(--border-subtle)] bg-[var(--surface-primary)] text-[var(--text-primary)]"><div className="mx-auto hidden max-w-7xl grid-cols-4 gap-12 px-8 py-14 md:grid"><div><BrandLogo className="mb-6 max-h-10 max-w-44 object-contain" textClassName="font-serif text-xl uppercase tracking-[0.15em]" /><div className="mt-6"><SocialLinks siteConfig={siteConfig} /></div></div>{sections.map((section) => <SectionLinks key={section.title} {...section} />)}<div><p className="mb-5 text-[10px] uppercase tracking-[0.2em]">{t('stayUpdated')}</p><p className="mb-5 text-xs opacity-70">{t('stayUpdatedDescription')}</p><Link to="/register" className="inline-flex border border-[var(--border-subtle)] px-5 py-3 text-[10px] uppercase tracking-widest">{t('subscribeNewsletter')}</Link><div className="mt-6"><SocialLinks siteConfig={siteConfig} /></div></div></div><div className="md:hidden"><div className="px-4">{sections.map((section) => <SectionLinks key={section.title} {...section} mobile />)}</div><div className="space-y-5 px-4 py-8"><Link to="/register" className="flex w-full justify-center border border-[var(--border-subtle)] py-3 text-[10px] uppercase tracking-widest">{t('subscribeNewsletter')}</Link><div className="flex justify-center"><SocialLinks siteConfig={siteConfig} /></div></div></div><div className="flex flex-col gap-3 border-t border-[var(--border-subtle)] px-4 py-5 text-center text-[9px] uppercase tracking-widest opacity-60 md:flex-row md:items-center md:justify-between md:px-8"><div className="flex flex-wrap justify-center gap-4 md:justify-start"><Link to="/legal/privacy">{t('privacyPolicy')}</Link><Link to="/legal/cookies">{t('cookiesPolicy')}</Link><Link to="/legal/terms">{t('terms')}</Link><Link to="/legal/modern-slavery">{t('modernSlavery')}</Link><button type="button" onClick={() => setFeedbackOpen(true)} className="hover:text-amber-300">{t('clientExperience')}</button></div><span>© 2026 {siteConfig.site_name || config.store_name || t('maison')} - {siteConfig.contact_phone} · {t('allRightsReserved')}</span></div></footer>{feedbackOpen && <SiteFeedbackModal onClose={() => setFeedbackOpen(false)} />}</>
}
