...
export default function Footer() {
  ...
  const sections = [
    ...
    { title: t('serviceSupport'), links: [[t('deliveryReturns'), '/legal/terms'], [t('careGuide'), '/legal/terms'], [t('bookAppointment'), '/concierge']] },
    { title: t('ourMaison'), links: [[t('ourStory'), '/'], [t('craftsmanship'), '/'], [t('careers'), '/careers']] },
  ]
  return <><footer className="border-t border-[var(--border-subtle)] bg-[var(--surface-primary)] text-[var(--text-primary)]"><div className="mx-auto hidden max-w-7xl grid-cols-4 gap-12 px-8 py-14 md:grid">{sections.map((section) => <SectionLinks key={section.title} {...section} />)}<div><p className="mb-5 text-[10px] uppercase tracking-[0.2em]">Stay up to date</p><p className="mb-5 text-xs opacity-70">Discover new creations and maison news.</p><button type="button" className="border border-[var(--border-subtle)] px-5 py-3 text-[10px] uppercase tracking-widest">Subscribe to newsletter</button><div className="mt-6"><SocialLinks siteConfig={siteConfig} /></div></div></div><div className="md:hidden"><div className="px-4">{sections.map((section) => <SectionLinks key={section.title} {...section} mobile />)}</div><div className="space-y-5 px-4 py-8"><button type="button" className="w-full border border-[var(--border-subtle)] py-3 text-[10px] uppercase tracking-widest">Subscribe to newsletter</button><div className="flex justify-center"><SocialLinks siteConfig={siteConfig} /></div></div></div><div className="flex flex-col gap-3 border-t border-[var(--border-subtle)] px-4 py-5 text-center text-[9px] uppercase tracking-widest opacity-60 md:flex-row md:items-center md:justify-between md:px-8"><div className="flex flex-wrap justify-center gap-4 md:justify-start"><Link to="/legal/privacy">Privacy Policy</Link><Link to="/legal/privacy">Cookies Policy</Link><Link to="/legal/terms">Terms</Link><Link to="/legal/terms">Modern Slavery Act</Link><button type="button" onClick={() => setFeedbackOpen(true)} className="hover:text-amber-300">Avis &amp; Experience Client</button></div><span>© 2026 {siteConfig.site_name || config.store_name || "Maison de L'Élégance"} - {siteConfig.contact_phone} · All rights reserved</span></div></footer>{feedbackOpen && <SiteFeedbackModal onClose={() => setFeedbackOpen(false)} />}</>
}
