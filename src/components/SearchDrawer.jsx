...
export default function SearchDrawer({ isOpen, onClose }) {
  ...
  return (
    <div className={`fixed inset-x-0 bottom-0 top-[60px] z-40 transition-opacity duration-300 ease-in-out md:top-0 ${isOpen ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'}`} aria-hidden={!isOpen}>
      <button type="button" aria-label="Close search" onClick={onClose} className="pointer-events-auto absolute inset-0 z-40 bg-black/60 transition-opacity duration-300 ease-in-out" />
      <aside className={`search-drawer pointer-events-auto fixed inset-x-0 bottom-0 top-0 z-[45] flex h-full transform flex-col overflow-y-auto bg-[var(--surface-primary)] text-[var(--text-primary)] shadow-2xl transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
