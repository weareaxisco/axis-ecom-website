        ...
        <div className="mt-12 grid gap-12 lg:grid-cols-[1fr_360px]">
          <section className="border-t border-neutral-800 pt-8">
            {step === 1 && <div className="space-y-6">
                    {hasOnsiteOnly ? <div className="border border-amber-500/50 bg-amber-500/10 p-5 text-sm leading-6 text-amber-200">{t('exclusivePickupMessage')}<strong className="mt-2 block text-xs uppercase tracking-widest text-amber-400">{t('onsitePickupPrice')}</strong></div> : <div className="border border-neutral-800 p-4"><label className="flex items-center gap-3 text-sm"><input type="radio" checked={fulfillment === 'delivery'} onChange={() => setFulfillment('delivery')} className="accent-amber-500" />{t('deliveryAcrossMorocco')}</label><p className="mt-2 pl-6 text-xs text-neutral-500">{t('selectCityPricing')}</p></div>}
              <div className="grid gap-5 sm:grid-cols-2">
                <label className="sm:col-span-2 text-[11px] uppercase tracking-widest text-neutral-400">{t('checkoutFullName')}<input value={form.fullName} onChange={update('fullName')} className={inputClass} /></label>
                <label className="sm:col-span-2 text-[11px] uppercase tracking-widest text-neutral-400">{t('checkoutAddress')}<input value={form.address} onChange={update('address')} className={inputClass} /></label>
                {!hasOnsiteOnly && <label htmlFor="checkout-city" className="text-[11px] uppercase tracking-widest text-neutral-400">{t('checkoutCity')}<select id="checkout-city" aria-label={t('checkoutCity')} value={form.city} onChange={update('city')} className={inputClass}>{cities.map((city) => <option key={city.city_id} value={city.city_name}>{city.city_name} — {city.delivery_price_dh} DH</option>)}</select><span className="mt-1 block text-[10px] font-mono text-amber-400">{selectedCity.delivery_price_dh} DH · {selectedCity.estimated_days} {t('dayDelivery')}</span></label>}
                {!hasOnsiteOnly && <label htmlFor="checkout-postal" className="text-[11px] uppercase tracking-widest text-neutral-400">{t('postalCode')}<input id="checkout-postal" value={form.postalCode} onChange={update('postalCode')} className={inputClass} /></label>}
                {!hasOnsiteOnly && <label htmlFor="checkout-phone" className="text-[11px] uppercase tracking-widest text-neutral-400">{t('checkoutPhone')}<input id="checkout-phone" aria-label={t('checkoutPhone')} value={form.phone} onChange={(event) => { update('phone')(event); setPhoneError('') }} className={inputClass} />{phoneError && <span className="mt-1 block text-xs text-rose-400">{phoneError}</span>}</label>}
                {hasOnsiteOnly && <><label className="text-[11px] uppercase tracking-widest text-neutral-400">{t('pickupDate')}<input type="date" value={form.pickupDate} onChange={update('pickupDate')} className={inputClass} /></label><label className="text-[11px] uppercase tracking-widest text-neutral-400">{t('checkoutTimeSlot')}<select value={form.pickupTime} onChange={update('pickupTime')} className={inputClass}><option value="">{t('selectTime')}</option><option>10:00 – 12:00</option><option>12:00 – 14:00</option><option>14:00 – 16:00</option><option>16:00 – 18:00</option></select></label></>}
              </div>
            </div>}
