      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-12 grid gap-12 lg:grid-cols-[1fr_360px]">
          <section className="border-t border-neutral-800 pt-8">
            ...
            {step === 3 && <div className="border border-amber-500/40 bg-neutral-900/50 p-8"><p className="text-center text-[10px] uppercase tracking-[0.25em] text-amber-400">Order Status</p><h2 className="mt-4 text-center font-serif text-2xl uppercase tracking-widest">Your journey with the Maison</h2><div className="mx-auto mt-8 max-w-xl space-y-5">{timeline.map((status, index) => <div key={status} className="flex gap-4"><span className={`mt-1 h-3 w-3 flex-shrink-0 rounded-full border ${index === 0 ? 'border-amber-400 bg-amber-400' : 'border-neutral-600'}`} /><div><p className={`text-xs uppercase tracking-widest ${index === 0 ? 'text-amber-400' : 'text-neutral-400'}`}>{status}</p>{payment === 'cod' && index === 1 && <button type="button" className="mt-2 text-[10px] uppercase tracking-widest text-amber-400 underline">Pay Upfront Deposit to Fast-Track Shipping</button>}</div></div>)}</div></div>}
            {step < 3 && <button type="button" onClick={advanceStep} className="mt-8 bg-amber-500 px-8 py-4 text-xs font-semibold uppercase tracking-widest text-black">{step === 1 ? 'Continue to Payment' : 'Review Order'}</button>}
          </section>
