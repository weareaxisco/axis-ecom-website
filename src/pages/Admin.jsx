...
function OrdersTable({ filteredOrders, visibleOrders, query, setQuery, page, setPage, pageSize, setPageSize, pageCount, onEdit, onDelete, onClear, onNotice }) {
  ...
  return <div className="relative">
    ...
    {preview && <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/70 p-4" role="dialog" aria-modal="true"><div className="max-h-[80vh] w-full max-w-2xl overflow-y-auto border border-neutral-700 bg-neutral-950 p-6"><div className="flex items-center justify-between"><h2 className="font-serif text-xl uppercase tracking-widest">Order Items</h2><button type="button" onClick={() => setPreview(null)} aria-label="Close preview"><X size={18} /></button></div><div className="mt-4 border-y border-neutral-800 py-4 text-xs text-neutral-400"><p className="text-sm text-white">{preview.customer_name || preview.full_name || 'Guest Customer'}</p><p className="mt-1">{preview.phone || '—'}</p><p className="mt-1">{preview.delivery_address || preview.shipping_address || '—'}</p><p className="mt-1">{preview.city || '—'} {preview.postal_code ? `· ${preview.postal_code}` : ''}</p></div><div className="mt-6 space-y-3">{(preview.items || []).map((item, index) => { const price = Number(item.price_dh || item.price || item.unit_price || 0); return <div key={`${item.id || item.product_id || item.name}-${index}`} className="flex items-center gap-4 border-b border-neutral-800 py-3"><img src={item.image || item.images?.[0] || '/placeholder.jpg'} alt="" className="h-16 w-16 object-cover" /><div className="min-w-0 flex-1"><a href={`/product/${item.id || item.product_id}`} className="text-sm text-amber-300 hover:text-white">{item.name || item.title || 'Product'}</a><p className="mt-1 text-xs text-neutral-500">Qty {item.quantity || 1} · {price.toLocaleString()} DH</p></div></div> })}</div><div className="mt-6 border-t border-neutral-800 pt-4 text-right text-sm text-amber-300">Order Total: {Number(preview.total_dh ?? preview.total_amount ?? preview.total ?? 0).toLocaleString()} DH</div></div></div>}
