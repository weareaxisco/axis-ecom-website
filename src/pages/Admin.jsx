...
const orderReference = (id) => {
  ...
  return `#ORD-${value.replace(/-/g, '').slice(0, 5).toUpperCase()}`
}
const normalizeOrderStatus = (status) => String(status || 'pending_confirmation').toLowerCase().replace(/\s+/g, '_')
...
const orderStatusClass = (status) => ({
  ...
  deposit_received: 'border-blue-500/30 bg-blue-500/10 text-blue-300',
  dispatched_ameex: 'border-purple-500/30 bg-purple-500/10 text-purple-300',
  in_preparation: 'border-purple-500/30 bg-purple-500/10 text-purple-300',
...
}[normalizeOrderStatus(status)] || 'border-neutral-700 bg-neutral-900 text-neutral-300')
...
function OrdersTable({ filteredOrders, visibleOrders, query, setQuery, page, setPage, pageSize, setPageSize, pageCount, onEdit, onDelete, onClear, onNotice }) {
  ...
  const saveEdit = async (event) => {
    ...
    setSaving(true)
    try {
      const items = (edit.items || []).filter((item) => Number(item.quantity) > 0).map((item) => ({ ...item, quantity: Number(item.quantity) }))
      const subtotal_dh = items.reduce((sum, item) => sum + Number(item.price_dh || item.price || item.unit_price || 0) * item.quantity, 0)
      await onEdit(edit.id, { customer_name: edit.customer_name, phone: edit.phone, delivery_address: edit.delivery_address, city: edit.city, status: edit.status, items, subtotal_dh, total_dh: subtotal_dh + Number(edit.shipping_fee_dh || 0) })
      setEdit(null)
    } catch (error) {
  ...
  return <div className="relative">
    ...
    <div className="overflow-x-auto border-x border-neutral-800 bg-neutral-950/70"><table className="w-full min-w-[1050px] text-left"><thead className="border-b border-neutral-800 text-[10px] uppercase tracking-[0.2em] text-neutral-500"><tr>{['Order ID', 'Customer Name', 'Items', 'City', 'Total', 'Status', 'Actions'].map((heading) => <th key={heading} className="px-5 py-4">{heading}</th>)}</tr></thead><tbody className="divide-y divide-neutral-800/80">{visibleOrders.map((order) => <tr key={order.id} className="text-sm"><td className="px-5 py-5 font-mono text-amber-400"><button type="button" title={order.id} onClick={() => navigator.clipboard?.writeText(order.id)} className="inline-flex items-center gap-2 hover:text-white">{orderReference(order.id)}<Copy size={13} /></button></td><td className="px-5 py-5">{order.customer_name || order.full_name || 'Guest Customer'}</td><td className="px-5 py-5"><button type="button" onClick={() => setPreview(order)} className="inline-flex items-center gap-1 text-xs uppercase tracking-widest text-amber-300 hover:text-white"><Eye size={14} /> {(order.items || []).length} items</button></td><td className="px-5 py-5 text-neutral-400">{order.city || '—'}</td><td className="px-5 py-5">{total(order).toLocaleString()} DH</td><td className="px-5 py-5 text-xs"><span className={`inline-flex border px-2 py-1 uppercase tracking-widest ${orderStatusClass(order.status)}`}>{order.status || 'Pending Confirmation'}</span></td><td className="px-5 py-5"><div className="flex gap-2"><button type="button" onClick={() => setEdit({ ...order, items: (order.items || []).map((item) => ({ ...item })), customer_name: order.customer_name || '', phone: order.phone || '', delivery_address: order.delivery_address || order.shipping_address || '', city: order.city || '', status: normalizeOrderStatus(order.status) })} className="inline-flex items-center gap-1 border border-neutral-700 px-2 py-1.5 text-[10px] uppercase tracking-widest hover:border-amber-400"><Edit3 size={13} /> Edit</button><button type="button" onClick={() => { if (window.confirm(`Permanently cancel and delete order ${orderReference(order.id)}?`)) onDelete(order.id).catch((error) => onNotice(`Unable to delete Order ${orderReference(order.id)}: ${error.message}`)) }} className="inline-flex items-center gap-1 border border-rose-500/50 px-2 py-1.5 text-[10px] uppercase tracking-widest text-rose-300 hover:border-rose-400"><Trash2 size={13} /> Delete</button></div></td></tr>)}</tbody></table>{!visibleOrders.length && <p className="p-10 text-center text-sm text-neutral-500">No orders found.</p>}</div>
    ...
    {preview && <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true"><div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto border border-amber-500/30 bg-neutral-950 p-6 text-neutral-200 shadow-2xl md:p-8"><div className="flex items-start justify-between gap-4 border-b border-neutral-800 pb-5"><div><p className="text-[10px] uppercase tracking-[0.3em] text-amber-400">Maison de l'Élégance</p><h2 className="mt-2 font-serif text-2xl uppercase tracking-widest text-amber-300">Détails de la Commande</h2><span className="mt-3 inline-flex border border-neutral-700 px-2 py-1 font-mono text-[10px] text-neutral-400">{orderReference(preview.id)}</span></div><div className="flex items-center gap-4"><span className="border border-amber-500/30 bg-amber-500/10 px-3 py-1 text-[10px] uppercase tracking-widest text-amber-300">{preview.status || 'En attente'}</span><button type="button" onClick={() => setPreview(null)} aria-label="Close preview" className="text-neutral-400 hover:text-amber-300"><X size={20} /></button></div></div><div className="mt-6 border border-amber-500/20 bg-neutral-900/80 p-5"><p className="text-[10px] uppercase tracking-widest text-neutral-500">Customer Details</p><div className="mt-3 grid gap-3 text-sm sm:grid-cols-3"><p><span className="block text-[10px] uppercase text-neutral-500">Client</span>{preview.customer_name || preview.full_name || 'Guest Customer'}</p><p><span className="block text-[10px] uppercase text-neutral-500">Phone</span>{preview.phone || '—'}</p><p className="sm:col-span-1"><span className="block text-[10px] uppercase text-neutral-500">Delivery</span>{preview.delivery_address || preview.shipping_address || '—'}{preview.city ? `, ${preview.city}` : ''}{preview.postal_code ? ` · ${preview.postal_code}` : ''}</p></div></div><div className="mt-6 space-y-3">{(preview.items || []).map((item, index) => { const price = Number(item.price_dh || item.price || item.unit_price || 0); const quantity = Number(item.quantity || 1); return <div key={`${item.id || item.product_id || item.name}-${index}`} className="flex items-center gap-4 border border-neutral-800 bg-neutral-900/40 p-4"><img src={item.image || item.images?.[0] || '/placeholder.jpg'} alt="" className="h-16 w-16 object-cover" /><div className="min-w-0 flex-1"><a href={`/product/${item.id || item.product_id}`} className="font-serif text-base text-amber-300 hover:text-white">{item.name || item.title || 'Product'}</a><p className="mt-2 text-xs text-neutral-500">{price.toLocaleString()} DH × {quantity}</p></div><p className="text-sm text-neutral-200">{(price * quantity).toLocaleString()} DH</p></div> })}</div><div className="mt-7 flex items-center justify-between border-t border-neutral-700 pt-5"><span className="text-[10px] uppercase tracking-[0.25em] text-neutral-500">Total Amount</span><strong className="text-2xl text-amber-300">{Number(preview.total_dh ?? preview.total_amount ?? preview.total ?? 0).toLocaleString()} DH</strong></div><button type="button" onClick={() => setPreview(null)} className="mt-6 w-full bg-amber-500 py-3 text-xs font-semibold uppercase tracking-widest text-black hover:bg-amber-400">Close Details</button></div></div>}
    {edit && <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/70 p-4" role="dialog" aria-modal="true"><form onSubmit={saveEdit} className="max-h-[90vh] w-full max-w-2xl overflow-y-auto border border-amber-500/30 bg-neutral-950 p-6 shadow-2xl"><div className="flex items-center justify-between border-b border-neutral-800 pb-4"><div><p className="text-[10px] uppercase tracking-widest text-amber-400">Order management</p><h2 className="mt-1 font-serif text-xl uppercase tracking-widest">Edit Order</h2></div><button type="button" onClick={() => setEdit(null)} aria-label="Close edit"><X size={18} /></button></div><div className="mt-6 grid gap-4 sm:grid-cols-2">{[['customer_name', 'Customer Name'], ['phone', 'Phone'], ['delivery_address', 'Address'], ['city', 'City']].map(([key, label]) => <label key={key} className="text-[10px] uppercase tracking-widest text-neutral-400">{label}<input value={edit[key]} onChange={(event) => setEdit((current) => ({ ...current, [key]: event.target.value }))} className="mt-1 w-full border border-neutral-800 bg-neutral-900 px-3 py-2 text-sm text-white outline-none focus:border-amber-500" /></label>)}</div><div className="mt-7"><p className="text-[10px] uppercase tracking-widest text-neutral-400">Order Items</p><div className="mt-3 space-y-2">{(edit.items || []).map((item, index) => <div key={`${item.id || item.product_id || item.name}-${index}`} className="flex items-center gap-3 border border-neutral-800 bg-neutral-900/60 p-3"><img src={item.image || item.images?.[0] || '/placeholder.jpg'} alt="" className="h-12 w-12 object-cover" /><span className="min-w-0 flex-1 text-sm text-amber-300">{item.name || item.title || 'Product'}</span><input aria-label={`Quantity for ${item.name || item.title || 'item'}`} type="number" min="0" value={item.quantity || 0} onChange={(event) => setEdit((current) => ({ ...current, items: current.items.map((currentItem, currentIndex) => currentIndex === index ? { ...currentItem, quantity: event.target.value } : currentItem) }))} className="w-20 border border-neutral-700 bg-neutral-950 px-2 py-2 text-center text-sm text-white" /><button type="button" onClick={() => setEdit((current) => ({ ...current, items: current.items.filter((_, currentIndex) => currentIndex !== index) }))} className="text-rose-300 hover:text-rose-200" aria-label={`Remove ${item.name || item.title || 'item'}`}><Trash2 size={16} /></button></div>)}</div></div><label className="mt-7 block text-[10px] uppercase tracking-widest text-neutral-400">Status<select value={normalizeOrderStatus(edit.status)} onChange={(event) => setEdit((current) => ({ ...current, status: event.target.value }))} className={`mt-2 w-full border px-3 py-2 text-sm outline-none ${orderStatusClass(edit.status)}`}>{orderStatuses.map((status) => <option key={status.value} value={status.value}>{status.label}</option>)}</select></label><button type="submit" disabled={saving} className="mt-7 w-full bg-amber-500 py-3 text-xs font-semibold uppercase tracking-widest text-black">{saving ? 'Saving…' : 'Save Changes'}</button></form></div>}
  </div>
}
...
function AdminContent() {
  ...
  const { t } = useLanguage()
  const { orders, updateOrder: contextUpdateOrder, deleteOrder, clearAllOrders } = useOrderContext()
  ...
  const canViewAnalytics = ['super_admin', 'admin'].includes(activeUser?.role)
  ...
  const updateOrder = useCallback(async (id, changes) => {
    ...
    const nextItems = changes.items || previousItems
    const quantities = (items) => items.reduce((result, item) => {
      const targetId = String(item.id || item.product_id || item.productId || '')
      result[targetId] = (result[targetId] || 0) + Number(item.quantity || 0)
      return result
    }, {})
    const previousQuantities = quantities(previousItems)
    const nextQuantities = quantities(nextItems)
    const shouldDecrement = ['confirmed', 'delivered'].includes(changes.status) && !order.stock_decremented
    const adjustStock = async (targetId, delta) => {
      ...
      const { error } = await supabase.from('products').update({ stock: newStock }).eq('id', targetId)
      if (error) throw error
      window.dispatchEvent(new CustomEvent('inventory_updated', { detail: { productId: targetId } }))
    }
    const shouldRestore = ['cancelled', 'refunded'].includes(changes.status) && order.stock_decremented && !order.stock_restored
    if (shouldDecrement) {
      for (const item of order.items || []) {
        const targetId = String(item.id || item.product_id || item.productId || '')
        const quantity = Number(item.quantity || 1)
        if (!targetId || !quantity) continue
        const { error: rpcError } = await supabase.rpc('decrement_product_stock', { p_id: targetId, qty: quantity })
        if (rpcError) {
          const { data: product, error: stockError } = await supabase.from('products').select('stock').eq('id', targetId).maybeSingle()
          if (stockError) throw stockError
          if (product) {
            const { error: updateError } = await supabase.from('products').update({ stock: Math.max(0, Number(product.stock) - quantity) }).eq('id', targetId)
            if (updateError) throw updateError
          }
        }
        window.dispatchEvent(new CustomEvent('inventory_updated', { detail: { productId: targetId } }))
      }
      await supabase.from('orders').update({ stock_decremented: true }).eq('id', id)
    }
    if (shouldRestore) {
      await Promise.all(Object.entries(previousQuantities).map(([targetId, quantity]) => adjustStock(targetId, Number(quantity))))
    } else if (order.stock_decremented && changes.items) {
      const targetIds = new Set([...Object.keys(previousQuantities), ...Object.keys(nextQuantities)])
      await Promise.all([...targetIds].map((targetId) => adjustStock(targetId, previousQuantities[targetId] - (nextQuantities[targetId] || 0))))
    }
    const subtotal_dh = changes.items
      ? nextItems.reduce((sum, item) => sum + Number(item.price_dh || item.price || item.unit_price || 0) * Number(item.quantity || 0), 0)
      : undefined
    return contextUpdateOrder(id, {
      ...changes,
      ...(subtotal_dh !== undefined ? { subtotal_dh, total_dh: subtotal_dh + Number(order.shipping_fee_dh || 0) } : {}),
      ...(shouldDecrement ? { stock_decremented: true } : {}),
      ...(shouldRestore ? { stock_restored: true } : {}),
    })
  }, [contextUpdateOrder, orders])
  ...
  const tabs = [
    ...(can('manage_orders') ? [{ id: 'orders', label: `${t('myOrders')} (${counts.orders})` }] : []),
    ...(canViewAnalytics ? [{ id: 'analytics', label: 'Analytics' }] : []),
    ...(can('manage_products') ? [{ id: 'inventory', label: `${t('inventory')} (${counts.products})` }] : []),
    ...(can('manage_taxonomies') ? [{ id: 'taxonomies', label: 'Taxonomies' }] : []),
    ...(can('manage_appointments') ? [{ id: 'appointments', label: `RENDEZ-VOUS (${appointmentCount})` }] : []),
