...
import { createSandboxParcel } from '../services/ameexApi'
...
export default function Admin() {
  ...
  const updateOrderStatus = async (id, status) => {
    ...
    try {
      let tracking
      if (status === 'dispatched_ameex' && !order?.ameex_tracking_id) tracking = await createSandboxParcel(order)
      const update = { status }
      ...
      if (tracking) setNotice(`Ameex Sandbox parcel created: ${tracking}`)
    } catch (error) {
