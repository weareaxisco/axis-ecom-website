    ...
    <div className="mx-auto max-w-7xl">
      ...
      <div className="mt-8 grid gap-8 md:grid-cols-[3fr_2fr]">
        <form onSubmit={save} className={`${activePanel === 'form' ? 'block' : 'hidden'} space-y-5 md:block`}>
          <div className="grid w-full grid-cols-1 gap-x-6 gap-y-5 border border-neutral-800 bg-neutral-950/60 p-6 md:grid-cols-2">
            <label className="text-[10px] uppercase tracking-widest text-neutral-400 md:col-span-2">Title<input required value={form.name} onChange={update('name')} className="mt-2 h-11 w-full border border-neutral-800 bg-neutral-900 px-3 text-sm outline-none focus:border-amber-400" /></label>
            <div className="grid w-full grid-cols-1 items-start gap-x-6 gap-y-5 md:grid-cols-2">
              <label className="block w-full text-xs uppercase tracking-wider text-neutral-400">SKU<input value={form.sku} onChange={update('sku')} placeholder="Auto-generated if empty" className="mt-2 h-11 w-full border border-neutral-800 bg-neutral-900 px-3 text-sm text-neutral-200 outline-none focus:border-amber-400" /></label>
              <div className="w-full"><TaxonomyCombobox label="Category" value={form.category} options={taxonomies.categories} onChange={(value) => setForm((current) => ({ ...current, category: value }))} /></div>
              <div className="w-full"><TaxonomyCombobox label="Collection" value={form.collection} options={taxonomies.collections} onChange={(value) => setForm((current) => ({ ...current, collection: value }))} /></div>
              <label className="block w-full text-xs uppercase tracking-wider text-neutral-400">PRICE (DH)<input required type="number" value={form.price_dh} onChange={update('price_dh')} className="mt-2 h-11 w-full border border-neutral-800 bg-neutral-900 px-3 text-sm text-neutral-200 outline-none focus:border-amber-400" /></label>
              <label className="block w-full text-xs uppercase tracking-wider text-neutral-400">STOCK<input type="number" value={form.stock} onChange={update('stock')} className="mt-2 h-11 w-full border border-neutral-800 bg-neutral-900 px-3 text-sm text-neutral-200 outline-none focus:border-amber-400" /></label>
              <div className="w-full"><TaxonomyCombobox label="Tags" value={tagDraft} options={taxonomies.tags} selectedValues={form.tags} onChange={setTagDraft} onRemove={(tag) => setForm((current) => ({ ...current, tags: current.tags.filter((item) => item !== tag) }))} onSelect={(tag) => { setForm((current) => ({ ...current, tags: current.tags.includes(tag) ? current.tags : [...current.tags, tag] })); setTagDraft('') }} /></div>
            </div>
