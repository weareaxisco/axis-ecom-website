      ...
      <div className="mx-auto max-w-3xl">
        ...
        <form className="mt-12 space-y-7" onSubmit={handleSubmit} noValidate>
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            {['firstName', 'lastName'].map((field) => <label key={field}><span className={labelClass}>{field === 'firstName' ? t('firstName') : t('lastName')}</span><input required autoComplete={field === 'firstName' ? 'given-name' : 'family-name'} value={form[field]} onChange={updateField(field)} className={inputClass} /><ErrorSlot message={errors[field]} /></label>)}
            <label><span className={labelClass}>{t('email')}</span><input type="email" autoComplete="email" value={form.email} onChange={updateField('email')} className={inputClass} /><ErrorSlot message={errors.email} /></label>
            ...
            <label><span className={labelClass}>{t('phone')}</span><input type="tel" autoComplete="tel" value={form.phone} onChange={updateField('phone')} placeholder="+212 6XX-XXXXXX" className={inputClass} /><small className="block text-xs text-neutral-500">{t('phoneFormatHint')}</small><ErrorSlot message={errors.phone} /></label>
            <label><span className={labelClass}>{t('password')}</span><div className="relative"><input type={showPassword ? 'text' : 'password'} autoComplete="new-password" value={form.password} onChange={updateField('password')} className={`${inputClass} pr-12`} /><button type="button" aria-label={t('togglePassword')} onClick={() => setShowPassword((value) => !value)} className="absolute right-3 top-3 text-neutral-400 hover:text-amber-400">{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div><ErrorSlot message={errors.password} /></label>
          </div>
        ...
        <div className="mt-12 w-full border-t border-neutral-800 bg-neutral-950/50 px-6 py-8 text-center"><p className="text-xs uppercase tracking-[0.2em] text-neutral-400">{t('alreadyHaveAccount')}</p><Link to="/login" className="mt-5 inline-flex w-full items-center justify-center border border-amber-500 px-5 py-4 text-xs font-semibold uppercase tracking-[0.2em] text-amber-400 transition-colors hover:bg-amber-500 hover:text-black">{t('login')}</Link></div>
