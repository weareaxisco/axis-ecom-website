...
export const exportToICS = (appointment) => {
  ...
  const description = `Client: ${appointment.fullName} (${appointment.email}, ${appointment.phone})\nFocus: ${appointment.focus}`
  const content = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Maison Diamiss//Concierge//EN', 'BEGIN:VEVENT', `UID:${appointment.id || generateUUID()}@maison-diamiss`, `DTSTAMP:${format(new Date())}`, `DTSTART:${format(start)}`, `DTEND:${format(end)}`, `SUMMARY:${escape('Consultation Privée - Maison Diamiss Kénitra')}`, `LOCATION:${escape('Maison Diamiss Joaillerie, Kénitra, Morocco')}`, `DESCRIPTION:${escape(description)}`, 'END:VEVENT', 'END:VCALENDAR'].join('\r\n')
