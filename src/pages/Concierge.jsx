...
export const getGoogleCalendarUrl = (appointment, locationUrl = '') => {
  const { start, end } = appointmentDateTimes(appointment)
  const params = new URLSearchParams({
    action: 'TEMPLATE',
    text: 'Private Visit - Maison Diamiss',
    dates: `${formatCalendarDate(start)}/${formatCalendarDate(end)}`,
    location: locationUrl || 'Maison Diamiss Joaillerie, Kénitra, Morocco',
    details: `Client: ${appointment.fullName || appointment.client_name} (${appointment.email}, ${appointment.phone})\nFocus: ${appointment.focus || appointment.service_type}`,
  })
  return `https://calendar.google.com/calendar/render?${params.toString()}`
}
export const getOutlookCalendarUrl = (appointment, locationUrl = '') => {
  const { start, end } = appointmentDateTimes(appointment)
  const params = new URLSearchParams({
    path: '/calendar/action/compose',
    rru: 'addevent',
    subject: 'Private Visit - Maison Diamiss',
    startdt: start.toISOString(),
    enddt: end.toISOString(),
    location: locationUrl || 'Maison Diamiss Joaillerie, Kénitra, Morocco',
    body: `Client: ${appointment.fullName || appointment.client_name} (${appointment.email}, ${appointment.phone})\nFocus: ${appointment.focus || appointment.service_type}`,
  })
